module atm_comp_esmf
  ...
  use cam_control_mod  , only: initial_run, cam_ctrl_set_orbit
  use radiation        , only: radiation_nextsw_cday
  use phys_grid        , only: get_ncols_p, get_gcol_all_p, get_gcol_p, ngcols, & 
                               get_rlat_all_p, get_rlon_all_p, get_area_all_p
  use ppgrid           , only: pcols, begchunk, endchunk       
  use dyn_grid         , only: get_horiz_grid_dim_d
  use camsrfexch       , only: cam_out_t, cam_in_t     
  use cam_initfiles    , only: cam_initfiles_get_caseid, cam_initfiles_get_restdir
  use filenames        , only: interpret_filename_spec
  use spmd_utils       , only: spmdinit, masterproc
...
CONTAINS
  ...
  subroutine atm_init_esmf(comp, import_state, export_state, EClock, rc)
    ...
    ! Locals
    !
    integer                    :: ATMID                ! atm component instance ID
    ...
    type(ESMF_Array)           :: a2x, x2a, dom
    type(ESMF_VM)              :: vm
    integer                    :: nflds, gsize, dsize
    ...
    if (first_time) then
       ...
       ! 
       ! Get attributes from export state
       !
       ...
       ! Add gsize attribute to export state

       call ESMF_AttributeSet(export_state, name="gsize", value=ngcols, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       ...
       distgrid = atm_distgrid_esmf()

       !-----------------------------------------
       !  Set arrayspec for dom, a2x and x2a
       !-----------------------------------------
       ...
       !-----------------------------------------
       ! Get dof for pio srf restarts
       !-----------------------------------------

       ! set dsize from elementcount
       call ESMF_DistGridGet(distgrid, localDe=0, elementCount=dsize, rc=rc)
       if(rc /= 0) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       ! allocate dof
       allocate(dof(dsize), stat=rc)

       ! set dof from seqindexlist
       ...
       nfields = shr_string_listGetNum(trim(seq_flds_a2x_fields))
       
       a2x = ESMF_ArrayCreate(distgrid=distgrid, arrayspec=arrayspec, distgridToArrayMap=(/2/), &
             undistLBound=(/1/), undistUBound=(/nfields/), name="d2x", rc=rc)
       if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call ESMF_AttributeSet(a2x, name="mct_names", value=trim(seq_flds_a2x_fields), rc=rc)
       if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       ...
       nfields = shr_string_listGetNum(trim(seq_flds_x2a_fields))

       x2a = ESMF_ArrayCreate(distgrid=distgrid, arrayspec=arrayspec, distgridToArrayMap=(/2/), &
             undistLBound=(/1/), undistUBound=(/nfields/), name="x2d", rc=rc)
       if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call ESMF_AttributeSet(x2a, name="mct_names", value=trim(seq_flds_x2a_fields), rc=rc)
       if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       ...
       call ESMF_StateAdd(export_state, (/a2x/), rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
    
       call ESMF_StateAdd(import_state, (/x2a/), rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       ! ---- Fill in atm_export state ----

       call ESMF_ArrayGet(a2x, localDe=0, farrayPtr=fptr, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call atm_export(cam_out, fptr)
       ...
       call ESMF_StateGet(export_state, itemName="d2x", array=a2x, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call ESMF_StateGet(import_state, itemName="x2d", array=x2a, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       ...
       if (StepNo == 0) then
          ...
          call atm_read_srfrest_esmf( EClock, x2a, a2x )

          call ESMF_ArrayGet(x2a, localDe=0, farrayPtr=fptr, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          call atm_import(fptr, cam_in, restart_init=.true.)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
...
!================================================================================
 ...
 subroutine atm_run_esmf(comp, import_state, export_state, EClock, rc)
    ...
    integer :: nstep           ! CAM nstep
    integer :: shrlogunit      ! old value
    integer :: shrloglev       ! old value
    real(r8):: caldayp1        ! CAM calendar day for for next cam time step
    real(r8):: nextsw_cday     ! calendar of next atm shortwave
    ...
    if (rstwr_sync) then
       call atm_write_srfrest_esmf( x2a, a2x, &
            yr_spec=yr_sync, mon_spec=mon_sync, day_spec=day_sync, sec_spec=tod_sync)
    end if
...
!================================================================================
  ...
  type(ESMF_DistGrid)  function atm_distgrid_esmf()
    ...
    integer :: i, n, c, ncols, sizebuf, rc
    !-------------------------------------------------------------------
    ...
    atm_distgrid_esmf = ESMF_DistGridCreate(arbSeqIndexList=gindex, rc=rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
  ...
  subroutine atm_read_srfrest_esmf( EClock, x2a, a2x)
  ...
  subroutine atm_write_srfrest_esmf(x2a, a2x, &
