...
!-----------------------------------------------------------------------
   ...
   use prognostics,  only: qm1, q3, t3, u3, v3, phis, omga, div, dps, dpsl, dpsm, &
   ...
   use scanslt,      only: plondfft, hw1, hw2, hw3, alpha, qfcst, hw1lat, engy1lat
   use spegrd,       only: spegrd_bft, spegrd_ift, spegrd_aft
...
! FFT buffers
!
   real(r8), allocatable:: fftbuf_in(:,:,:,:)  ! fftbuf_in(nlon_fft_in,11,plevp,plat) 
   real(r8), allocatable:: fftbuf_out(:,:,:,:) ! fftbuf_out(nlon_fft_out,11,plevp,beglat:endlat) 
!
!-----------------------------------------------------------------------
!
   call t_startf ('grcalc_init')
grdpss(:,:)       = nan
grpss(:,:)        = nan
grpls(:,:)        = nan
grpms(:,:)        = nan
...
!$OMP PARALLEL DO PRIVATE (LAT, J)
...
#if ( defined SPMD )
   nlon_fft_in = 2*maxm
   allocate(fftbuf_in(nlon_fft_in,11,plevp,plat))
#else
   nlon_fft_in = 1
   ...
   allocate(fftbuf_out(nlon_fft_out,11,plevp,beglat:endlat))
!
   fftbuf_in(:,:,:,:) = nan
   fftbuf_out(:,:,:,:) = nan
...
!$OMP PARALLEL DO PRIVATE (LAT, J)
   ...
   do lat=beglat,endlat
      ...
      call spegrd_aft (ztodt, lat, plon, nlon_fft_out, &
                       ...
                       qm(1,1,lat), t3(1,1,j,n3) ,engy2alat(lat), engy2blat(lat), &
