...
!========================================================================
  ...
  subroutine spmd_readnl(nlfilename, dyn_npes)
    ...
    use spmd_utils,      only: mpi_integer, mpicom, npes, mpi_success
...
!   Note that only dyn_npes is currently used by the SE dycore
    ...
    call mpi_bcast (dyn_npes, 1, mpi_integer, 0, mpicom, ierr)
    if (ierr /= mpi_success) then
      call endrun('spmd_readnl: bcast of dyn_npes failed')
    end if
