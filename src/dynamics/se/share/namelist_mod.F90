  ...
  subroutine homme_postprocess_namelist(mesh_file, par)
    ...
    if (NThreads*vert_num_threads > omp_get_max_threads()) then
      if(par%masterproc) then
        ...
        write(iulog, *) 'omp_get_max_threads() = ',OMP_get_max_threads()
        write(iulog, *) 'requested threads exceeds OMP_get_max_threads()'
    ...
    if (hypervis_subcycle == -1) then
      if (np == 4) then
        ...
        call abortmp('hypervis_subcycle auto determine only supported for np==4')
      end if
    end if
#endif
! ^ifndef CAM
    ...
    if ((mesh_file /= "none") .AND. (ne /= 0)) then
      if(par%masterproc) then
        write(iulog, *) "namelist_mod: mesh_file:",mesh_file,                 &
    ...
    if (ne .eq. 0) then
      if (mesh_file == "none") then
        call abortmp("NE = 0 indicates refined mesh but no mesh file entered")
      else if (par%masterproc) then
    ...
    if (par%masterproc) then
      write(iulog, *) "Reference element projection: cubed_sphere_map=",cubed_sphere_map
    end if
    ...
    if (hypervis_power /= 0) then
      if (hypervis_scaling /= 0) then
        if(par%masterproc) then
          write(iulog, *) 'Both hypervis_power and hypervis_scaling are nonzero.'
          write(iulog, *) '(1) Set hypervis_power=3.322, hypervis_scaling=0 for HV based on an element area.'
          write(iulog, *) '(2) Set hypervis_power=0 and hypervis_scaling=3.2 for HV based on a tensor.'
    ...
    if (tstep_type == 0) then
      ...
      if (ftype > 0) then
        call abortmp('adjustment type forcing (ftype>0) not allowed with tstep_type=0')
      end if
      if (qsplit > 1) then
    ...
    if (phys_tscale /= 0) then
      if (ftype > 0) then
        call abortmp('user specified se_phys_tscale requires se_ftype<=0')
      end if
    end if
...
! ============================================
  subroutine readnl(par)
    use control_mod,     only: pertlim
    use interpolate_mod, only: vector_uvars, vector_vvars, max_vecvars
    use interpolate_mod, only: interpolate_analysis, replace_vec_by_vordiv
    ...
    ! ==========================
    ! Set the default values for HOMME
    ...
    if (par%masterproc) then
...
#else
      write(iulog, *)  'Reading namelist ctl_nl from standard input'
      read(*,nml=ctl_nl)
#endif
#ifndef _USEMETIS
      !=================================
      ...
      if (integration == "semi_imp") then
        ...
        write(iulog, *) 'HYPERVIS order = ',hypervis_order
      ...
      ! =======================
      ! vert_nl namelist (note, PRIMDG always reads, PRIM depending on test)
      ! =======================
#ifdef _PRIMDG
      ...
      if ( test_case(1:12) == "held_suarez0"  .or. &
    ...
    call fvm_get_test_type(cslam_ideal_test, cslam_test_type, fvm_ideal_test, fvm_test_type)
    ...
    call homme_postprocess_namelist(mesh_file, par)
