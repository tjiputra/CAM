...
subroutine grid_vars_init(pi, ae, om, dt, fft_flt, grid)
...
!$omp  parallel do private(i,j,k)
      do k=1,km
        do j=jlast, jfirst+1, -1
          do i=ifirst,ilast
            u(i,j,k) = 0.5_r8*(u(i,j-1,k) + u(i,j,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i, k)
         do k=1,km
            do i=ifirst,ilast
              u(i,jfirst,k) = 0.5_r8 * ( u(i,jfirst,k) + usouth(i,k) )
            enddo
         enddo
      endif
#endif
      ...
      do k=1,km
         do j=jfirst, jlast
            do i=ilast,ifirst+1,-1
               v(i,j,k) = 0.5_r8*(v(i-1,j,k) + v(i,j,k))
            enddo
         enddo
      enddo
!
...
!$omp  parallel do private(i,j,k)
      do k=1,km
         do j=jfirst, jlast
            v(ifirst,j,k)= 0.5_r8*(vwest(j,k) + v(ifirst,j,k))
         enddo
      enddo
...
!$omp  parallel do private(i, k)
         ...
         do k=1,km
            do i=ifirst,ilast
               ua(i,jlast,k) = 0.5_r8 * ( u(i,jlast,k) + unorth(i,k) )
            enddo
         enddo
      endif
#endif
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
        do j=jfirst, jlast-1
          do i=ifirst,ilast
            ua(i,j,k) = 0.5_r8*(u(i,j,k) + u(i,j+1,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
         do j=jfirst, jlast
            do i=ifirst,ilast-1
               va(i,j,k) = 0.5_r8*(v(i,j,k) + v(i+1,j,k))
            enddo
            va(ilast,j,k) = 0.5_r8*(v(ilast,j,k) + veast(j,k))
         enddo
      enddo
...
!$omp  parallel do private(i, k)
         ...
         do k=1,km
            do i=ifirst,ilast
               vb(i,jfirst,k) = 0.5_r8 * ( v(i,jfirst,k) + vsouth(i,k) )
            enddo
         enddo
      endif
#endif
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
        do j=jfirst+1, jlast
          do i=ifirst,ilast
            vb(i,j,k) = 0.5_r8*(v(i,j,k) + v(i,j-1,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
         do j=jfirst, jlast
            do i=ifirst,ilast-1
               ub(i,j,k) = 0.5_r8*(u(i,j,k) + u(i+1,j,k))
            enddo
            ub(ilast,j,k) = 0.5_r8*(u(ilast,j,k) + ueast(j,k))
         enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      do k=1,km
        do j=jlast, jfirst+1, -1
          do i=ifirst,ilast
            v(i,j,k) = 0.5_r8*(v(i,j-1,k) + v(i,j,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i, k)
         do k=1,km
            do i=ifirst,ilast
              v(i,jfirst,k) = 0.5_r8 * ( v(i,jfirst,k) + vsouth(i,k) )
            enddo
         enddo
      endif
#endif
...
!$omp  parallel do private(i,j,k)
      do k=1,km
         do j=jfirst, jlast
            do i=ilast,ifirst+1,-1
               u(i,j,k) = 0.5_r8*(u(i-1,j,k) + u(i,j,k))
            enddo
         enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      do k=1,km
         do j=jfirst, jlast
            u(ifirst,j,k)= 0.5_r8*(uwest(j,k) + u(ifirst,j,k))
         enddo
      enddo
...
!$omp  parallel do private(i, k)
         ...
         do k=1,km
            do i=ifirst,ilast
               va(i,jlast,k) = 0.5_r8 * ( v(i,jlast,k) + vnorth(i,k) )
            enddo
         enddo
      endif
#endif
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
        do j=jfirst, jlast-1
          do i=ifirst,ilast
            va(i,j,k) = 0.5_r8*(v(i,j,k) + v(i,j+1,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,km
         do j=jfirst, jlast
            do i=ifirst,ilast-1
               ua(i,j,k) = 0.5_r8*(u(i,j,k) + u(i+1,j,k))
            enddo
            ua(ilast,j,k) = 0.5_r8*(u(ilast,j,k) + ueast(j,k))
         enddo
      enddo
