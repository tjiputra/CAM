...
!
    ...
    call infld(fieldname, fh_ini, 'lon', 'slat', 'lev',  ifirstxy, ilastxy, jfirstxy, jlastxy, 1, km, &
        dyn_in%u3s, readvar, gridname='fv_u_stagger')

    if(.not. readvar) call endrun()

    fieldname = 'VS'
    call infld(fieldname, fh_ini, 'slon', 'lat', 'lev',  ifirstxy, ilastxy, jfirstxy, jlastxy, 1, km, &
         dyn_in%v3s, readvar, gridname='fv_v_stagger')
    if(.not. readvar) call endrun()

    fieldname = 'T'

    call infld(fieldname, fh_ini, 'lon', 'lat', 'lev', ifirstxy, ilastxy, jfirstxy, jlastxy, 1, km, &
    ...
    case ('CONSTS')
	  ...
	  call get_block_gcol_d(iam+1,blksiz,gcid)
          ...
          if (microp_driver_implements_cnst(cnst_name(m_cnst))) then
             ...
             call rk_stratiform_init_cnst(cnst_name(m_cnst),q3tmp , gcid)
