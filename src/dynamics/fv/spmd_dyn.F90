...
!BOC
   ...
   if (modc_sw_dynrun .gt. 0 .and. modcomm_transpose .gt. 0) then
      modc_sw_dynrun = 0
      if (masterproc) then
   ...
   if (modc_sw_cdcore .gt. 0 .and. (modcomm_transpose .gt. 0 .or. (modcomm_geopk .gt. 0 .and. geopk16byte))) then
      modc_sw_cdcore = 0
      if (masterproc) then
   ...
   if (modc_sw_gather .gt. 0 .and. modcomm_gatscat .gt. 0) then
      modc_sw_gather = 0
      if (masterproc) then
   ...
   if (modc_sw_scatter .gt. 0 .and. modcomm_gatscat .gt. 0) then
      modc_sw_scatter = 0
      if (masterproc) then
   ...
   if (modc_sw_tracer .gt. 0 .and. modcomm_transpose .gt. 0) then
      modc_sw_tracer = 0
      if (masterproc) then
