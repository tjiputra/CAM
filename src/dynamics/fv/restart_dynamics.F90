...
!----------------------------------------------------------------------- 
! 
! Read and write dynamics fields to restart file
!
! !HISTORY:
...
#if ( defined OFFLINE_DYN )
   ! This write happens here rather than from write_restart_dynamics because
   ! only attributes are written (two filenames) and that should happen during
   ! the file definition phase.
...
!++bee it does not appear that this time variable is read by read_restart_dynamics.  is it read somewhere else?
...
subroutine read_restart_dynamics(File, dyn_in, dyn_out)
   ...
   integer :: beglonxy, endlonxy, beglatxy, endlatxy
   integer :: i, s2d, s3d
...
subroutine set_r_var(name, timelevels, index, v2, v3, v4, v5, v2r4, v3r4, v4r4, v5r4)
   ...
   if (present(v2)) then
      ...
      call endrun('set_r_var: ERROR: bad ndims')
   end if
...
subroutine init_restart_varlistw(dyn_out)
   ...
   if (vcnt /= restartvarcnt) then
      write(iulog,*) 'init_restart_varlistw: ERROR: vcnt= ', vcnt, &
                     ' restartvarcnt=', restartvarcnt
      call endrun('init_restart_varlistw: ERROR: bad restartvarcnt')
   end if
...
subroutine init_restart_varlistr(dyn_in)
   ...
   if (vcnt /= restartvarcnt) then
      write(iulog,*) 'init_restart_varlistr: ERROR: vcnt= ', vcnt, &
                     ' restartvarcnt=',restartvarcnt
      call endrun('init_restart_varlistr: ERROR: bad restartvarcnt')
   end if
