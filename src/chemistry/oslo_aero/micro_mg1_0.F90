...
subroutine micro_mg_tend ( &
     ...
     relvar, accre_enhan,                             &
     icecldf, cldo, rate1ord_cw2pr_st, naai,          &
     !++ classnuc wy
...
!++ classnuc wy
   real(r8), intent(in) :: frzimm(pcols,pver), frzcnt(pcols,pver), frzdep(pcols,pver)
...
!-- classnuc wy
    ...
    if (.not. classnuc_in) then
        ...
        dum = ((-mnuccc(k)-mnucct(k)-mnudep(k)-msacwi(k))*lcldm(i,k)+(prci(k)+ &
               prai(k))*icldm(i,k))*deltat
    end if
    !--classnuc wy
            if (dum.gt.qie) then
        ...
        if (.not. classnuc_in) then
            ...
            ratio = (qie/deltat+(mnuccc(k)+mnucct(k)+mnudep(k)+msacwi(k))*lcldm(i,k))/((prci(k)+prai(k))*icldm(i,k))*omsm 
        end if
               prci(k) = prci(k)*ratio
               prai(k) = prai(k)*ratio
            end if
    ...
    if (.not. classnuc_in) then
        ...
        dum = ((-nnucct(k)-nnuccc(k)-nnudep(k)-nsacwi(k))*lcldm(i,k)+(nprci(k)+ &
               nprai(k)-nsubi(k))*icldm(i,k))*deltat
    end if
    !--classnuc wy
            if (dum.gt.nie) then
       ...
       !++classnuc wy
        if (.not. classnuc_in) then
            ...
            ratio = (nie/deltat+(nnucct(k)+nnuccc(k)+nnudep(k)+nsacwi(k))*lcldm(i,k))/ &  
                    ((nprci(k)+nprai(k)-nsubi(k))*icldm(i,k))*omsm
        end if
	...
	qitend(i,k) = qitend(i,k)+ &
         (mnuccc(k)+mnucct(k)+mnudep(k)+msacwi(k))*lcldm(i,k)+(-prci(k)- & 
                  prai(k))*icldm(i,k)+cmei(i,k)+berg(i,k)
    end if
    !--classnuc wy
         end if
    ...
    if (.not. classnuc_in) then
	    ...
	    nitend(i,k) = nitend(i,k)+ nnuccd(k)*mtime+ & 
                  (nnucct(k)+nnuccc(k)+nnudep(k)+nsacwi(k))*lcldm(i,k)+(nsubi(k)-nprci(k)- &   
                  nprai(k))*icldm(i,k)
    end if
    !--classnuc wy
         end if
