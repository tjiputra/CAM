...
use calcaersize,       only: calcaersize_sub
...
use oslo_dust_intr,    only: oslo_dust_emis_intr, oslo_dust_initialize
use oslo_salt_intr,    only: oslo_salt_emis_intr, oslo_salt_initialize
...
use oslo_aerosols_intr, only: oslo_aero_dry_intr, oslo_aero_wet_intr
#endif
...
#ifdef OSLO_AERO
   call oslo_dust_initialize(dust_emis_fact, soil_erod)
   call oslo_salt_initialize()
#endif
...
! for the mozart aerosols -- fracis is used in deep convection routine
   ...
   !Initialize values to keep interface to wetdep routine..
   oslo_dgnumwet(:,:,:) = 0.1e-6_r8 !0.1 um

   call pbuf_get_field(pbuf, rate1_cw2pr_st_idx, rate1ord_cw2pr_st)

   !Sub cloud coefficients are hard-coded (for now) in the oslo scheme
   !So don't need the diameters here!
   !call calcaersize_sub( ncol, &
   !                  state%t, state%q(1,1,1), state%pmid, state%pdel &
   !                  ,oslo_dgnumwet , oslo_wetdens )

   call oslo_aero_wet_intr (state, ptend, nstep, dt, cme, prain, &
            evapr, cldv, cldvcu, cldvst, cldc, cldn, fracis, calday, cmfdqr, evapc, conicw, rainmr, &
            rate1ord_cw2pr_st, &   ! rce 2010/05/01
            oslo_dgnumwet, cam_out, dlf, pbuf)
...
subroutine aerosol_drydep_intr (state, ptend, cam_in, cam_out, dt, &
   ...
   call oslo_aero_dry_intr(state, ptend, wvflx, dt, lat, clat, &
            fsds, obklen, ts, ustar, prect, snowh, pblh, hflx, mon, &
            landfrac, icefrac, ocnfrac, cam_in%fv, cam_in%ram1, &
            oslo_dgnumwet, oslo_wetdens,                        &
            oslo_dgnumwet_processmodes, oslo_wetdens_processmodes, &
