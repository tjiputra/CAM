...
#ifdef OSLO_AERO
  use oslo_salt_intr, only : OMOceanSource  !kg/m2/sec
...
contains
  ...
  subroutine set_srf_emissions( rlats, rlons, lchnk, sflx, ncol )
    ...
    emis_loop : do m = 1,n_emis_species
...
#if (defined OSLO_AERO)
          elseif (emissions(m)%species == 'DMS') then
             if (oslo_dms_inq())then
      ...
      !At this point, emissions are in kg/m2/s ==> add Ocean OM source
#if (defined OSLO_AERO)
      select case( emissions(m)%species )
      case( 'OM_NI' )
          sflx(:ncol,n) = sflx(:ncol,n) + OMOceanSource(:ncol)
          !print*, "adding spracklen source " , sum(spracklenOMOceanSource(:ncol)), sum(sflx(:ncol,n)), 100.0*sum(spracklenOMOceanSource(:ncol))/(1.e-200 +  sum(sflx(:ncol,n)))
      end select
#endif
