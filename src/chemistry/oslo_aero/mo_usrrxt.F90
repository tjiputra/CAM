...
!-----------------------------------------------------------------
...
#ifdef OSLO_AERO
    real(r8), target  :: sfc_array(pcols,pver,nmodes_oslo), dm_array(pcols,pver,nmodes_oslo)
#elif (defined MODAL_AERO)
    ...
    if( usr_NO2_aer_ndx > 0 .or. usr_NO3_aer_ndx > 0 .or. usr_N2O5_aer_ndx > 0 .or. usr_HO2_aer_ndx > 0 ) then
       if( carma_do_hetchem ) then
         ...
         call surfarea( mmr, pmid, temp, ncol, sfc_array, dm_array, sad_total)
#elif (defined MODAL_AERO)
...
#ifdef OSLO_AERO
  !-------------------------------------------------------------------------
  ! provides aerosol surface area info for oslo aerosols
  !-------------------------------------------------------------------------
  subroutine surfarea( mmr, pmid, temp, ncol, sfc, dm_aer, sad_total )
!    use aerosoldef, only : nmodes_oslo => nmodes 
    use commondefinitions, only: nmodes_oslo => nmodes 
    ...
    real(r8), intent(in)    :: temp(:,:)           ![K] temperature
    real(r8), intent(in)    :: mmr(:,:,:)          ![kg/kg] mass mixing ratio
