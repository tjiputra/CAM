...
subroutine micro_mg_tend ( &
     ...
     rate1ord_cw2pr_st,  naain,                                  &
     !++ classnuc wy
     frzimm, frzcnt, frzdep,                                     &
     !-- classnuc wy
     npccnin,  rndstn,   naconin,  &
...
!  use microp_aero, only: frzimm_idx, frzcnt_idx, frzdep_idx, ntot_amode
!-- classnuc wy
  ...
  integer nstep
!++ classnuc wy
   real(r8), intent(in) :: frzimm(:,:), frzcnt(:,:), frzdep(:,:)
   real(r8) :: frzimm_part(mgncol,nlev), frzcnt_part(mgncol,nlev), frzdep_part(mgncol,nlev)
   real(r8) :: mnudep(mgncol,nlev) ! mixing ratio tendency due to deposition of water vapor
   real(r8) :: nnudep(mgncol,nlev) ! number conc tendency due to deposition of water vapor
...
!++ classnuc wy
if(classnuc_in) then
    call pack_array(      frzimm, mgcols, top_lev,       frzimm_part)
    call pack_array(      frzcnt, mgcols, top_lev,       frzcnt_part)
    call pack_array(      frzdep, mgcols, top_lev,       frzdep_part)
end if
!-- classnuc wy
 ...
 if(.not. classnuc_in) then
    ...
    if(do_cldice) then
      do i = 1,mgncol
         ...
         r3lx = con1*(rho(i,k)*qcic(i,k)/(rhow*max(ncic(i,k)*rho(i,k), 1.0e6_r8)))**0.333 ! in m
         r3lx = max(4.e-6, r3lx)
	 mi0l = 4._r8/3._r8*pi*rhow*r3lx**3_r8
                
         nnuccc(i,k) = frzimm_part(i,k)*1.0e6_r8/rho(i,k)
         mnuccc(i,k) = nnuccc(i,k)*mi0l 

         nnucct(i,k) = frzcnt_part(i,k)*1.0e6_r8/rho(i,k)
         mnucct(i,k) = nnucct(i,k)*mi0l 

         nnudep(i,k) = frzdep_part(i,k)*1.0e6_r8/rho(i,k)
          ...
          !-- classnuc wy
              ...
              if (dum.gt.qie) then
               ...
               if(.not. classnuc_in) then
                 ...
                 ratio = (qie/deltat+(mnuccc(i,k)+mnucct(i,k)+mnudep(i,k)+msacwi(i,k))*lcldm(i,k))/&
                       ((prci(i,k)+prai(i,k))*icldm(i,k))*omsm 
               end if
               ! classnuc wy
           ...
           if(.not. classnuc_in) then
              ...
              dum = ((-nnucct(i,k)-nnuccc(i,k)-nnudep(i,k)-nsacwi(i,k))*lcldm(i,k)+(nprci(i,k)+ &
                   nprai(i,k)-nsubi(i,k))*icldm(i,k))*deltat
           end if
           !-- classnuc wy
              ...
              if (dum.gt.nie) then
               ...
               if(.not. classnuc_in) then
                 ...
                 ratio = (nie/deltat+(nnucct(i,k)+nnuccc(i,k)+nnudep(i,k)+nsacwi(i,k))*lcldm(i,k))/ &  
                      ((nprci(i,k)+nprai(i,k)-nsubi(i,k))*icldm(i,k))*omsm
               end if
               !-- classnuc wy
           ...
           if(.not. classnuc_in) then
              ...
              nitend(i,k) = nitend(i,k)+ nnuccd(i,k)*mtime+ & 
                  (nnucct(i,k)+nnuccc(i,k)+nnudep(i,k)+nsacwi(i,k))*lcldm(i,k)+(nsubi(i,k)-nprci(i,k)- &   
                  nprai(i,k))*icldm(i,k)
           end if
           !-- classnuc wy
           end if
