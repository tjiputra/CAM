...
#if ( defined OSLO_AERO )
      integer, target    ::  spc_ids(6)
      integer, pointer   ::  id_so2, id_so4_1, &
...
#if (defined MODAL_AERO) || (defined OSLO_AERO)
#if ( defined OSLO_AERO )
      !Link the numbers actually in memory (id_XX) to pointer address
      !spc_ids. This is not setting any values to the numbers!!
      id_so2    => spc_ids(1)
      id_so4_1  => spc_ids(2)
      id_h2o2   => spc_ids(3)
      id_ox     => spc_ids(4)
...
#if (defined MODAL_AERO) || (defined OSLO_AERO)
#if (defined OSLO_AERO)
      id_so4_1     =  l_so4_a2            !in physics-space (from aerosoldef)
#elif ( defined MODAL_AERO_7MODE )
...
#endif
        real(r8)                :: qcwin(ncol, pver)   !NOTE ncol in dimension!!
        real(r8)                :: totprod(ncol,pver)
        !-----------------------------------------------------------------------      
        !      ... Local variables
        !
...
#if (defined MODAL_AERO) || (defined OSLO_AERO)
#if (defined OSLO_AERO)
        id_so4_1a = id_so4_1 - loffset ! ==> index in mozart-space
        qcwin(:,:) = qcw(:,:,id_so4_1a)
#else
...
#if (defined OSLO_AERO)
           !has values because of call to qqcw2vmr in mo_gas_phase_chemdr
           !and that works because of call to qqcw_set_ptr in aerosoldef!
...
#endif
#else
                 fact1_nh3 = (xk*xe*patm/xkw)*xnh3(i,k)
#endif
...
#if (defined MODAL_AERO) || (defined OSLO_AERO)
                    write(iulog,'(2a,i10,2i5,1p,3e12.4)') &
                       'SETSOX: pH failed to converge -- ', &
                       'lchnk, i, k, yph_lo/mid/hi =', lchnk, i, k, yph_lo, yph, yph_hi
#else
...
!END OF OSLO_AERO
#else
      !In the case of OSLO-AEROSOLS, set no MSA in cloud droplets
      ntot_msa_c = 0
#endif
!-----------------------------------------------------------------------
...
#endif
#else
           !BULK AERO, MAKE SURE THIS IS NOT COMPILED WHEN OSLO_AERO
#ifndef OSLO_AERO
           if (.not. inv_so2) then
