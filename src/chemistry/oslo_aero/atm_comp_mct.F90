...
#ifdef OSLO_AERO
      !Should improve the fraction of tracer going to which mode
      do nmode = 1, getNumberOfDustModes()

         !This is the index of the dust tracer in this mode
         tracerIndex = getDustTracerIndexInMode(nmode)

         !dust emission module keeps track of what part of total flux that goes
         !to what mode...
         cam_in(c)%cflx(i,tracerIndex) = getEmissionFractionInDustMode(nmode) &
                                      * (-x2a_a%rAttr(index_x2a_Fall_flxdst1, ig) &
                                         -x2a_a%rAttr(index_x2a_Fall_flxdst2, ig) &
                                         -x2a_a%rAttr(index_x2a_Fall_flxdst3, ig) &
                                         -x2a_a%rAttr(index_x2a_Fall_flxdst4, ig))
