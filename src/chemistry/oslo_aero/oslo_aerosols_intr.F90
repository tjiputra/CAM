...
contains
  ...
  subroutine oslo_aero_initialize(pbuf2d )
    ...
    do m=0,nmodes
       do l=1,getNumberOfTracersInMode(m)
          ...
          call addfld (trim(aName)//'SFWET',unit_basename//'/m2/s ',1,  'A','Wet deposition flux at surface',phys_decomp)
          call addfld (trim(aName)//'SFSIC',unit_basename//'/m2/s ', &
               1,  'A','Wet deposition flux (incloud, convective) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSIS',unit_basename//'/m2/s ', &
               1,  'A','Wet deposition flux (incloud, stratiform) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSBC',unit_basename//'/m2/s ', &
               1,  'A','Wet deposition flux (belowcloud, convective) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSBS',unit_basename//'/m2/s ', &
               1,  'A','Wet deposition flux (belowcloud, stratiform) at surface',phys_decomp)
          call addfld (trim(aName)//'WET',unit_basename//'/kg/s ',pver, 'A','wet deposition tendency',phys_decomp)
          call addfld (trim(aName)//'SIC',unit_basename//'/kg/s ',pver, 'A', &
               trim(aName)//' ic wet deposition',phys_decomp)
          call addfld (trim(aName)//'SIS',unit_basename//'/kg/s ',pver, 'A', &
               trim(aName)//' is wet deposition',phys_decomp)
          call addfld (trim(aName)//'SBC',unit_basename//'/kg/s ',pver, 'A', &
               trim(aName)//' bc wet deposition',phys_decomp)
          call addfld (trim(aName)//'SBS',unit_basename//'/kg/s ',pver, 'A', &
               trim(aName)//' bs wet deposition',phys_decomp)
          ...
          !ddep output
          call addfld (trim(aName)//'DDF',unit_basename//'/m2/s ',   1, 'A', &
               trim(aName)//' dry deposition flux at bottom (grav + turb)',phys_decomp)
          call addfld (trim(aName)//'TBF',unit_basename//'/m2/s',   1, 'A', &
               trim(aName)//' turbulent dry deposition flux',phys_decomp)
          call addfld (trim(aName)//'GVF',unit_basename//'/m2/s ',   1, 'A', &
               trim(aName)//' gravitational dry deposition flux',phys_decomp)
          call addfld (trim(aName)//'DTQ',unit_basename//'/kg/s ',pver, 'A', &
               trim(aName)//' dry deposition',phys_decomp)
          call addfld (trim(aName)//'DDV','m/s     ',pver, 'A', &
               trim(aName)//' deposition velocity',phys_decomp)
          ...
          call addfld (trim(aName)//'SFWET', unit_basename//'/m2/s ',1,  'A', &
              trim(aName)//' wet deposition flux at surface',phys_decomp)
          call addfld (trim(aName)//'SFSIC', unit_basename//'/m2/s ',1,  'A', &
              trim(aName)//' wet deposition flux (incloud, convective) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSIS', unit_basename//'/m2/s ',1,  'A', &
               trim(aName)//' wet deposition flux (incloud, stratiform) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSBC', unit_basename//'/m2/s ',1,  'A', &
               trim(aName)//' wet deposition flux (belowcloud, convective) at surface',phys_decomp)
          call addfld (trim(aName)//'SFSBS', unit_basename//'/m2/s ',1,  'A', &
               trim(aName)//' wet deposition flux (belowcloud, stratiform) at surface',phys_decomp)
          !dry deposition
          call addfld (trim(aName)//'DDF',   unit_basename//'/m2/s ',   1, 'A', &
               trim(aName)//' dry deposition flux at bottom (grav + turb)',phys_decomp)
          call addfld (trim(aName)//'TBF',   unit_basename//'/m2/s ',   1, 'A', &
  ...
  !===============================================================================
  subroutine oslo_aero_wet_intr ( state, ptend, nstep, dt, cme, prain,    &
    ...
    real(r8), intent(in) :: dgncur_awet(pcols,pver,0:nmodes) ! wet/ambient geom. mean diameter (m)
...
!         !         sol_factic = 0.4_r8   ! tuned 1/6
          ...
          if ((lphase == 1) .and. (lspec <= getNumberOfTracersInMode(m))) then
             ...
             if (.not. associated(fldcw))then
               fldcw=>zeroAerosolConcentration
             endif
...
! MAM3 code on aerosol water recalculation deleted. CAM-Oslo diagnoses aerosol
! cloud water from hygroscopic growth. The hygroscopic growth depends on the mixtures
  ...
  !===============================================================================
  subroutine oslo_aero_dry_intr ( state, ptend, wvflx, dt, lat, clat, &
       fsds, obklen, ts, ustar, prect, snowh, pblh, hflx, month, landfrac, &
       icefrac, ocnfrac, fvin,ram1in, dgncur_awet, wetdens,                &
       dgncur_awet_processmode, wetdens_processmode,                       &
       cflx,                                                               &
       cam_out, pbuf)
    !-----------------------------------------------------------------------
    !
