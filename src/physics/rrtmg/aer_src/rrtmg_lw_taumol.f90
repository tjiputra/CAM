...
! ------- Modules -------
      ...
      use parrrtm,  only: nbndlw, maxxsec, ngptlw
...
! ----- Input -----
      ...
      real(kind=r8), intent(in) :: pavel(nlayers)           ! layer pressures (mb) 
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: wx(maxxsec,nlayers)            ! cross-section amounts (mol/cm2)
                                                      !    Dimensions: (maxxsec,nlayers)
      real(kind=r8), intent(in) :: coldry(nlayers)          ! column amount (dry air)
                                                      !    Dimensions: (nlayers)

      integer, intent(in) :: laytrop         ! tropopause layer index
      integer, intent(in) :: jp(nlayers)           ! 
                                                      !    Dimensions: (nlayers)
      integer, intent(in) :: jt(nlayers)           !
                                                      !    Dimensions: (nlayers)
      integer, intent(in) :: jt1(nlayers)          !
                                                      !    Dimensions: (nlayers)
      ...
      real(kind=r8), intent(in) :: colh2o(nlayers)          ! column amount (h2o)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colco2(nlayers)          ! column amount (co2)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colo3(nlayers)           ! column amount (o3)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: coln2o(nlayers)          ! column amount (n2o)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colco(nlayers)           ! column amount (co)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colch4(nlayers)          ! column amount (ch4)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colo2(nlayers)           ! column amount (o2)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colbrd(nlayers)          ! column amount (broadening gases)
                                                      !    Dimensions: (nlayers)

      integer, intent(in) :: indself(nlayers)
                                                      !    Dimensions: (nlayers)
      integer, intent(in) :: indfor(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: selffac(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: selffrac(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: forfac(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: forfrac(nlayers)
                                                      !    Dimensions: (nlayers)

      integer, intent(in) :: indminor(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: minorfrac(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: scaleminor(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: scaleminorn2(nlayers)
                                                      !    Dimensions: (nlayers)

      real(kind=r8), dimension(nlayers), intent(in) :: &                  !
...
! ----- Output -----
      real(kind=r8), intent(out) :: fracs(nlayers,ngptlw)        ! planck fractions
                                                      !    Dimensions: (nlayers,ngptlw)
      real(kind=r8), intent(out) :: taug(nlayers,ngptlw)         ! gaseous optical depth 
                                                      !    Dimensions: (nlayers,ngptlw)
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      real(kind=r8) ::  corradj(nlayers), scalen2(nlayers), tauself, taufor, taun2
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         !pp = pavel(lay)
         corradj(lay) =  1.
         scalen2(lay) = colbrd(lay) * scaleminorn2(lay)
      enddo
      do lay = 1, laytrop
         if (pavel(lay) .lt. 250._r8) then
            corradj(lay) = 1._r8 - 0.15_r8 * (250._r8-pavel(lay)) / 154.4_r8
         endif
      enddo
      do lay = 1, laytrop
         do ig = 1, ng1
            ...
            taun2 = scalen2(lay)*(ka_mn2(indminor(lay),ig) + & 
                 minorfrac(lay) * (ka_mn2(indminor(lay)+1,ig) - ka_mn2(indminor(lay),ig)))
            taug(lay,ig) = corradj(lay) * (colh2o(lay) * &
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         ...
         !indm = indminor(lay)
         !pp = pavel(lay)
         corradj(lay) =  1._r8 - 0.15_r8 * (pavel(lay) / 95.6_r8)
      ...
      do lay = laytrop+1, nlayers
         do ig = 1, ng1
            ...
            taun2 = scalen2(lay)*(kb_mn2(indminor(lay),ig) + & 
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      real(kind=r8) :: corradj(nlayers), tauself, taufor
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         !indf = indfor(lay)
         !pp = pavel(lay)
         corradj(lay) = 1._r8 - .05_r8 * (pavel(lay) - 100._r8) / 900._r8
      enddo
      do lay = 1, laytrop
         do ig = 1, ng2
            tauself = selffac(lay) * (selfref(indself(lay),ig) + selffrac(lay) * &
            ...
            taufor =  forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            taug(lay,ngs1+ig) = corradj(lay) * (colh2o(lay) * &
                (fac00(lay) * absa(ind0(lay),ig) + &
                 fac10(lay) * absa(ind0(lay)+1,ig) + &
                 fac01(lay) * absa(ind1(lay),ig) + &
                 fac11(lay) * absa(ind1(lay)+1,ig)) &
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers),  ig
      integer, dimension(nlayers) :: js, js1, jmn2o, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mn2o, specparm_mn2o, specmult_mn2o, &
                       fmn2o, fmn2omf, chi_n2o, ratn2o, adjfac, adjcoln2o
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb(lay) = colh2o(lay) + rat_h2oco2(lay)*colco2(lay)
         specparm(lay) = colh2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 8._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         fs(lay) = mod(specmult(lay),1.0_r8)        

         speccomb1(lay) = colh2o(lay) + rat_h2oco2_1(lay)*colco2(lay)
         specparm1(lay) = colh2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 8._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         fs1(lay) = mod(specmult1(lay),1.0_r8)

         speccomb_mn2o(lay) = colh2o(lay) + refrat_m_a*colco2(lay)
         specparm_mn2o(lay) = colh2o(lay)/speccomb_mn2o(lay)
         if (specparm_mn2o(lay) .ge. oneminus) specparm_mn2o(lay) = oneminus
         specmult_mn2o(lay) = 8._r8*specparm_mn2o(lay)
         jmn2o(lay) = 1 + int(specmult_mn2o(lay))
         fmn2o(lay) = mod(specmult_mn2o(lay),1.0_r8)
         fmn2omf(lay) = minorfrac(lay)*fmn2o(lay)
!  In atmospheres where the amount of N2O is too great to be considered
!  a minor species, adjust the column amount of N2O by an empirical factor 
!  to obtain the proper contribution.
         chi_n2o(lay) = coln2o(lay)/coldry(lay)
         ratn2o(lay) = 1.e20_r8*chi_n2o(lay)/chi_mls(4,jp(lay)+1)
      enddo
      do lay = 1, laytrop
         if (ratn2o(lay) .gt. 1.5_r8) then
            adjfac(lay) = 0.5_r8+(ratn2o(lay)-0.5_r8)**0.65_r8
            adjcoln2o(lay) = adjfac(lay)*chi_mls(4,jp(lay)+1)*coldry(lay)*1.e-20_r8
         else
      ...
      do lay = 1, laytrop
         ...
         speccomb_planck(lay) = colh2o(lay)+refrat_planck_a*colco2(lay)
         specparm_planck(lay) = colh2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 8._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         fpl(lay) = mod(specmult_planck(lay),1.0_r8)

         ind0(lay) = ((jp(lay)-1)*5+(jt(lay)-1))*nspa(3) + js(lay)
         ind1(lay) = (jp(lay)*5+(jt1(lay)-1))*nspa(3) + js1(lay)
      enddo
      do lay = 1, laytrop
         ...
         if (specparm(lay) .lt. 0.125_r8) then
            ...
            fk1 = 1 - p - 2.0_r8*p4
            fk2 = p + p4
            fac000(lay) = fk0*fac00(lay)
            fac100(lay) = fk1*fac00(lay)
            fac200(lay) = fk2*fac00(lay)
            fac010(lay) = fk0*fac10(lay)
            fac110(lay) = fk1*fac10(lay)
            fac210(lay) = fk2*fac10(lay)
         else if (specparm(lay) .gt. 0.875_r8) then
            p = -fs(lay) 
            p4 = p**4
            fk0 = p4
            fk1 = 1 - p - 2.0_r8*p4
            ...
            fac000(lay) = (1._r8 - fs(lay)) * fac00(lay)
            fac010(lay) = (1._r8 - fs(lay)) * fac10(lay)
            fac100(lay) = fs(lay) * fac00(lay)
         ...
         if (specparm1(lay) .lt. 0.125_r8) then
            ...
            fk1 = 1 - p - 2.0_r8*p4
            fk2 = p + p4
            fac001(lay) = fk0*fac01(lay)
            fac101(lay) = fk1*fac01(lay)
            fac201(lay) = fk2*fac01(lay)
            fac011(lay) = fk0*fac11(lay)
            fac111(lay) = fk1*fac11(lay)
            fac211(lay) = fk2*fac11(lay)
         else if (specparm1(lay) .gt. 0.875_r8) then
            p = -fs1(lay) 
            p4 = p**4
            fk0 = p4
            fk1 = 1 - p - 2.0_r8*p4
            ...
            fac001(lay) = (1._r8 - fs1(lay)) * fac01(lay)
            fac011(lay) = (1._r8 - fs1(lay)) * fac11(lay)
            fac101(lay) = fs1(lay) * fac01(lay)
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng3
            ...
            taufor = forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            n2om1 = ka_mn2o(jmn2o(lay),indminor(lay),ig) + fmn2o(lay) * &
                 (ka_mn2o(jmn2o(lay)+1,indminor(lay),ig) - ka_mn2o(jmn2o(lay),indminor(lay),ig))
            n2om2 = ka_mn2o(jmn2o(lay),indminor(lay)+1,ig) + fmn2o(lay) * &
                 (ka_mn2o(jmn2o(lay)+1,indminor(lay)+1,ig) - ka_mn2o(jmn2o(lay),indminor(lay)+1,ig))
            absn2o = n2om1 + minorfrac(lay) * (n2om2 - n2om1)

            if (specparm(lay) .lt. 0.125_r8) then
               tau_major = speccomb(lay) * &
                    (fac000(lay) * absa(ind0(lay),ig) + &
                    fac100(lay) * absa(ind0(lay)+1,ig) + &
                    fac200(lay) * absa(ind0(lay)+2,ig) + &
                    fac010(lay) * absa(ind0(lay)+9,ig) + &
                    fac110(lay) * absa(ind0(lay)+10,ig) + &
                    fac210(lay) * absa(ind0(lay)+11,ig))
            else if (specparm(lay) .gt. 0.875_r8) then
               tau_major = speccomb(lay) * &
                    (fac200(lay) * absa(ind0(lay)-1,ig) + &
                    fac100(lay) * absa(ind0(lay),ig) + &
                    fac000(lay) * absa(ind0(lay)+1,ig) + &
                    fac210(lay) * absa(ind0(lay)+8,ig) + &
                    fac110(lay) * absa(ind0(lay)+9,ig) + &
                    fac010(lay) * absa(ind0(lay)+10,ig))
            else
               tau_major = speccomb(lay) * &
                    (fac000(lay) * absa(ind0(lay),ig) + &
                    fac100(lay) * absa(ind0(lay)+1,ig) + &
            ...
            if (specparm1(lay) .lt. 0.125_r8) then
               tau_major1 = speccomb1(lay) * &
                    (fac001(lay) * absa(ind1(lay),ig) + &
                    fac101(lay) * absa(ind1(lay)+1,ig) + &
                    fac201(lay) * absa(ind1(lay)+2,ig) + &
                    fac011(lay) * absa(ind1(lay)+9,ig) + &
                    fac111(lay) * absa(ind1(lay)+10,ig) + &
                    fac211(lay) * absa(ind1(lay)+11,ig))
            else if (specparm1(lay) .gt. 0.875_r8) then
               tau_major1 = speccomb1(lay) * &
                    (fac201(lay) * absa(ind1(lay)-1,ig) + &
                    fac101(lay) * absa(ind1(lay),ig) + &
                    fac001(lay) * absa(ind1(lay)+1,ig) + &
                    fac211(lay) * absa(ind1(lay)+8,ig) + &
                    fac111(lay) * absa(ind1(lay)+9,ig) + &
                    fac011(lay) * absa(ind1(lay)+10,ig))
            else
               tau_major1 = speccomb1(lay) * &
                    (fac001(lay) * absa(ind1(lay),ig) +  &
                    fac101(lay) * absa(ind1(lay)+1,ig) + &
            ...
            fracs(lay,ngs2+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         ...
         specparm(lay) = colh2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 4._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         fs(lay) = mod(specmult(lay),1.0_r8)
         ...
         specparm1(lay) = colh2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 4._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         fs1(lay) = mod(specmult1(lay),1.0_r8)
         ...
         speccomb_mn2o(lay) = colh2o(lay) + refrat_m_b*colco2(lay)
         specparm_mn2o(lay) = colh2o(lay)/speccomb_mn2o(lay)
         if (specparm_mn2o(lay) .ge. oneminus) specparm_mn2o(lay) = oneminus
         specmult_mn2o(lay) = 4._r8*specparm_mn2o(lay)
         jmn2o(lay) = 1 + int(specmult_mn2o(lay))
         fmn2o(lay) = mod(specmult_mn2o(lay),1.0_r8)
         fmn2omf(lay) = minorfrac(lay)*fmn2o(lay)
!  In atmospheres where the amount of N2O is too great to be considered
...
!  to obtain the proper contribution.
         chi_n2o(lay) = coln2o(lay)/coldry(lay)
         ratn2o(lay) = 1.e20*chi_n2o(lay)/chi_mls(4,jp(lay)+1)
      enddo
      do lay = laytrop+1, nlayers
         if (ratn2o(lay) .gt. 1.5_r8) then
            adjfac(lay) = 0.5_r8+(ratn2o(lay)-0.5_r8)**0.65_r8
      ...
      do lay = laytrop+1, nlayers
         ...
         speccomb_planck(lay) = colh2o(lay)+refrat_planck_b*colco2(lay)
         specparm_planck(lay) = colh2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 4._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         fpl(lay) = mod(specmult_planck(lay),1.0_r8)

         ind0(lay) = ((jp(lay)-13)*5+(jt(lay)-1))*nspb(3) + js(lay)
         ind1(lay) = ((jp(lay)-12)*5+(jt1(lay)-1))*nspb(3) + js1(lay)
      enddo
      do lay = laytrop+1, nlayers
         ...
         do ig = 1, ng3
            taufor = forfac(lay) * (forref(indfor(lay),ig) + &
                 forfrac(lay) * (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            n2om1 = kb_mn2o(jmn2o(lay),indminor(lay),ig) + fmn2o(lay) * &
                 (kb_mn2o(jmn2o(lay)+1,indminor(lay),ig)-kb_mn2o(jmn2o(lay),indminor(lay),ig))
            n2om2 = kb_mn2o(jmn2o(lay),indminor(lay)+1,ig) + fmn2o(lay) * &
                 (kb_mn2o(jmn2o(lay)+1,indminor(lay)+1,ig)-kb_mn2o(jmn2o(lay),indminor(lay)+1,ig))
            absn2o = n2om1 + minorfrac(lay) * (n2om2 - n2om1)
            taug(lay,ngs2+ig) = speccomb(lay) * &
                (fac000(lay) * absb(ind0(lay),ig) + &
                fac100(lay) * absb(ind0(lay)+1,ig) + &
                fac010(lay) * absb(ind0(lay)+5,ig) + &
                fac110(lay) * absb(ind0(lay)+6,ig)) &
                + speccomb1(lay) * &
                (fac001(lay) * absb(ind1(lay),ig) +  &
                fac101(lay) * absb(ind1(lay)+1,ig) + &
                fac011(lay) * absb(ind1(lay)+5,ig) + &
                fac111(lay) * absb(ind1(lay)+6,ig))  &
                + taufor &
                + adjcoln2o(lay)*absn2o
            fracs(lay,ngs2+ig) = fracrefb(ig,jpl(lay)) + fpl(lay) * &
                (fracrefb(ig,jpl(lay)+1)-fracrefb(ig,jpl(lay)))
         enddo
      enddo
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      integer, dimension(nlayers) :: js, js1, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng4
            ...
            taug(lay,ngs3+ig) = tau_major + tau_major1 &
                 + tauself + taufor
            fracs(lay,ngs3+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
! Upper atmosphere loop
      ...
      do lay = laytrop+1, nlayers
         ...
         specparm(lay) = colo3(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 4._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         fs(lay) = mod(specmult(lay),1.0_r8)

         speccomb1(lay) = colo3(lay) + rat_o3co2_1(lay)*colco2(lay)
         specparm1(lay) = colo3(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 4._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         fs1(lay) = mod(specmult1(lay),1.0_r8)
         ...
         speccomb_planck(lay) = colo3(lay)+refrat_planck_b*colco2(lay)
         specparm_planck(lay) = colo3(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 4._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         fpl(lay) = mod(specmult_planck(lay),1.0_r8)

         ind0(lay) = ((jp(lay)-13)*5+(jt(lay)-1))*nspb(4) + js(lay)
         ind1(lay) = ((jp(lay)-12)*5+(jt1(lay)-1))*nspb(4) + js1(lay)
         do ig = 1, ng4
            taug(lay,ngs3+ig) =  speccomb(lay) * &
                (fac000(lay) * absb(ind0(lay),ig) + &
                fac100(lay) * absb(ind0(lay)+1,ig) + &
                fac010(lay) * absb(ind0(lay)+5,ig) + &
                fac110(lay) * absb(ind0(lay)+6,ig)) &
                ...
                fac011(lay) * absb(ind1(lay)+5,ig) + &
                fac111(lay) * absb(ind1(lay)+6,ig))
            fracs(lay,ngs3+ig) = fracrefb(ig,jpl(lay)) + fpl(lay) * &
                (fracrefb(ig,jpl(lay)+1)-fracrefb(ig,jpl(lay)))
         enddo
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers),  ig
      integer, dimension(nlayers) :: js, js1, jmo3, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mo3, specparm_mo3, specmult_mo3, fmo3
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb_mo3(lay) = colh2o(lay) + refrat_m_a*colco2(lay)
         specparm_mo3(lay) = colh2o(lay)/speccomb_mo3(lay)
         if (specparm_mo3(lay) .ge. oneminus) specparm_mo3(lay) = oneminus
         specmult_mo3(lay) = 8._r8*specparm_mo3(lay)
         jmo3(lay) = 1 + int(specmult_mo3(lay))
         fmo3(lay) = mod(specmult_mo3(lay),1.0_r8)
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng5
            ...
            taufor =  forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            o3m1 = ka_mo3(jmo3(lay),indminor(lay),ig) + fmo3(lay) * &
                 (ka_mo3(jmo3(lay)+1,indminor(lay),ig)-ka_mo3(jmo3(lay),indminor(lay),ig))
            o3m2 = ka_mo3(jmo3(lay),indminor(lay)+1,ig) + fmo3(lay) * &
                 (ka_mo3(jmo3(lay)+1,indminor(lay)+1,ig)-ka_mo3(jmo3(lay),indminor(lay)+1,ig))
            ...
            taug(lay,ngs4+ig) = tau_major + tau_major1 &
                 ...
                 + abso3*colo3(lay) &
                 + wx(1,lay) * ccl4(ig)
            fracs(lay,ngs4+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
      ...
      do lay = laytrop+1, nlayers
         do ig = 1, ng5
            taug(lay,ngs4+ig) = speccomb(lay) * &
                ...
                fac111(lay) * absb(ind1(lay)+6,ig))  &
                + wx(1,lay) * ccl4(ig)
            fracs(lay,ngs4+ig) = fracrefb(ig,jpl(lay)) + fpl(lay) * &
                (fracrefb(ig,jpl(lay)+1)-fracrefb(ig,jpl(lay)))
         enddo
      enddo
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      real(kind=r8), dimension(nlayers) :: chi_co2, ratco2, adjfac, adjcolco2
...
! to obtain the proper contribution.
      ...
      do lay = 1, laytrop
         if (ratco2(lay) .gt. 3.0_r8) then
            ...
            adjcolco2(lay) = adjfac(lay)*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_r8
         else
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      integer, dimension(nlayers) :: js, js1, jmco2, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mco2, specparm_mco2, specmult_mco2, fmco2
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb(lay) = colh2o(lay) + rat_h2oo3(lay)*colo3(lay)
         specparm(lay) = colh2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 8._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         ...
         speccomb1(lay) = colh2o(lay) + rat_h2oo3_1(lay)*colo3(lay)
         specparm1(lay) = colh2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 8._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         ...
         speccomb_mco2(lay) = colh2o(lay) + refrat_m_a*colo3(lay)
         specparm_mco2(lay) = colh2o(lay)/speccomb_mco2(lay)
         if (specparm_mco2(lay) .ge. oneminus) specparm_mco2(lay) = oneminus
         specmult_mco2(lay) = 8._r8*specparm_mco2(lay)

         jmco2(lay) = 1 + int(specmult_mco2(lay))
         fmco2(lay) = mod(specmult_mco2(lay),1.0_r8)

!  In atmospheres where the amount of CO2 is too great to be considered
!  a minor species, adjust the column amount of CO2 by an empirical factor 
!  to obtain the proper contribution.
         chi_co2(lay) = colco2(lay)/(coldry(lay))
         ratco2(lay) = 1.e20*chi_co2(lay)/chi_mls(2,jp(lay)+1)
         if (ratco2(lay) .gt. 3.0_r8) then
            adjfac(lay) = 3.0_r8+(ratco2(lay)-3.0_r8)**0.79_r8
            adjcolco2(lay) = adjfac(lay)*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_r8
         else
            adjcolco2(lay) = colco2(lay)
         endif

         speccomb_planck(lay) = colh2o(lay)+refrat_planck_a*colo3(lay)
         specparm_planck(lay) = colh2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 8._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         ...
         ind0(lay) = ((jp(lay)-1)*5+(jt(lay)-1))*nspa(7) + js(lay)
         ind1(lay) = (jp(lay)*5+(jt1(lay)-1))*nspa(7) + js1(lay)
         ...
         do ig = 1, ng7
            ...
            taufor = forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            co2m1 = ka_mco2(jmco2(lay),indminor(lay),ig) + fmco2(lay) * &
                 (ka_mco2(jmco2(lay)+1,indminor(lay),ig) - ka_mco2(jmco2(lay),indminor(lay),ig))
            co2m2 = ka_mco2(jmco2(lay),indminor(lay)+1,ig) + fmco2(lay) * &
                 (ka_mco2(jmco2(lay)+1,indminor(lay)+1,ig) - ka_mco2(jmco2(lay),indminor(lay)+1,ig))
            ...
            taug(lay,ngs6+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + adjcolco2(lay)*absco2
            fracs(lay,ngs6+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
!  to obtain the proper contribution.
         ...
         ratco2(lay) = 1.e20*chi_co2(lay)/chi_mls(2,jp(lay)+1)
         if (ratco2(lay) .gt. 3.0_r8) then
            adjfac(lay) = 2.0_r8+(ratco2(lay)-2.0_r8)**0.79_r8
            adjcolco2(lay) = adjfac(lay)*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_r8
         else
            adjcolco2(lay) = colco2(lay)
         endif
...
!  to obtain the proper contribution.
         ...
         ratco2(lay) = 1.e20_r8*chi_co2(lay)/chi_mls(2,jp(lay)+1)
         if (ratco2(lay) .gt. 3.0_r8) then
            adjfac(lay) = 2.0_r8+(ratco2(lay)-2.0_r8)**0.65_r8
            adjcolco2(lay) = adjfac(lay)*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_r8
         else
            adjcolco2(lay) = colco2(lay)
         endif
...
! Local 
      ...
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mn2o, specparm_mn2o, specmult_mn2o, fmn2o
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=r8) :: tauself, taufor, n2om1, n2om2, absn2o
      real(kind=r8), dimension(nlayers) :: chi_n2o, ratn2o, adjfac, adjcoln2o
      real(kind=r8) :: refrat_planck_a, refrat_m_a
      real(kind=r8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb(lay) = colh2o(lay) + rat_h2och4(lay)*colch4(lay)
         specparm(lay) = colh2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 8._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         ...
         speccomb1(lay) = colh2o(lay) + rat_h2och4_1(lay)*colch4(lay)
         specparm1(lay) = colh2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 8._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         ...
         speccomb_mn2o(lay) = colh2o(lay) + refrat_m_a*colch4(lay)
         specparm_mn2o(lay) = colh2o(lay)/speccomb_mn2o(lay)
         if (specparm_mn2o(lay) .ge. oneminus) specparm_mn2o(lay) = oneminus
         specmult_mn2o(lay) = 8._r8*specparm_mn2o(lay)
         jmn2o(lay) = 1 + int(specmult_mn2o(lay))
...
!  to obtain the proper contribution.
         ...
         speccomb_planck(lay) = colh2o(lay)+refrat_planck_a*colch4(lay)
         specparm_planck(lay) = colh2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 8._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         ...
         ind0(lay) = ((jp(lay)-1)*5+(jt(lay)-1))*nspa(9) + js(lay)
         ind1(lay) = (jp(lay)*5+(jt1(lay)-1))*nspa(9) + js1(lay)
         ...
         do ig = 1, ng9
            ...
            taug(lay,ngs8+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + adjcoln2o(lay)*absn2o
            fracs(lay,ngs8+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      real(kind=r8) :: scaleo2(nlayers), tauself, taufor, tauo2
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         scaleo2(lay) = colo2(lay)*scaleminor(lay)
         do ig = 1, ng11
            tauself = selffac(lay) * (selfref(indself(lay),ig) + selffrac(lay) * &
                 (selfref(indself(lay)+1,ig) - selfref(indself(lay),ig)))
            taufor = forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig)))
            tauo2 =  scaleo2(lay) * (ka_mo2(indminor(lay),ig) + minorfrac(lay) * &
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         ...
         do ig = 1, ng11
            taufor = forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            tauo2 =  scaleo2(lay) * (kb_mo2(indminor(lay),ig) + minorfrac(lay) * &
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng12
            ...
            taug(lay,ngs11+ig) = tau_major + tau_major1 &
                 + tauself + taufor
            fracs(lay,ngs11+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      integer, dimension(nlayers) :: js, js1, jmco2, jmco, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mco2, specparm_mco2, specmult_mco2, fmco2
      real(kind=r8), dimension(nlayers) :: speccomb_mco, specparm_mco, specmult_mco, fmco
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb(lay) = colh2o(lay) + rat_h2on2o(lay)*coln2o(lay)
         specparm(lay) = colh2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 8._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         ...
         speccomb1(lay) = colh2o(lay) + rat_h2on2o_1(lay)*coln2o(lay)
         specparm1(lay) = colh2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 8._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         ...
         speccomb_mco2(lay) = colh2o(lay) + refrat_m_a*coln2o(lay)
         specparm_mco2(lay) = colh2o(lay)/speccomb_mco2(lay)
         if (specparm_mco2(lay) .ge. oneminus) specparm_mco2(lay) = oneminus
         specmult_mco2(lay) = 8._r8*specparm_mco2(lay)
         jmco2(lay) = 1 + int(specmult_mco2(lay))
...
!  to obtain the proper contribution.
      ...
      do lay = 1, laytrop
         if (ratco2(lay) .gt. 3.0_r8) then
            ...
            adjcolco2(lay) = adjfac(lay)*3.55e-4*coldry(lay)*1.e-20_r8
         else
            adjcolco2(lay) = colco2(lay)
         endif
      ...
      do lay = 1, laytrop
         ...
         if (specparm_mco(lay) .ge. oneminus) specparm_mco(lay) = oneminus
         specmult_mco(lay) = 8._r8*specparm_mco(lay)
         jmco(lay) = 1 + int(specmult_mco(lay))
         fmco(lay) = mod(specmult_mco(lay),1.0_r8)

         speccomb_planck(lay) = colh2o(lay)+refrat_planck_a*coln2o(lay)
         specparm_planck(lay) = colh2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 8._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng13
            ...
            co2m2 = ka_mco2(jmco2(lay),indminor(lay)+1,ig) + fmco2(lay) * &
                 (ka_mco2(jmco2(lay)+1,indminor(lay)+1,ig) - ka_mco2(jmco2(lay),indminor(lay)+1,ig))
            absco2 = co2m1 + minorfrac(lay) * (co2m2 - co2m1)
            com1 = ka_mco(jmco(lay),indminor(lay),ig) + fmco(lay) * &
                 (ka_mco(jmco(lay)+1,indminor(lay),ig) - ka_mco(jmco(lay),indminor(lay),ig))
            com2 = ka_mco(jmco(lay),indminor(lay)+1,ig) + fmco(lay) * &
                 (ka_mco(jmco(lay)+1,indminor(lay)+1,ig) - ka_mco(jmco(lay),indminor(lay)+1,ig))
            ...
            taug(lay,ngs12+ig) = tau_major + tau_major1 &
                 ...
                 + adjcolco2(lay)*absco2 &
                 + colco(lay)*absco
            fracs(lay,ngs12+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         do ig = 1, ng13
            abso3 = kb_mo3(indminor(lay),ig) + minorfrac(lay) * &
                 (kb_mo3(indminor(lay)+1,ig) - kb_mo3(indminor(lay),ig))
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ind0(lay) = ((jp(lay)-1)*5+(jt(lay)-1))*nspa(14) + 1
         ind1(lay) = (jp(lay)*5+(jt1(lay)-1))*nspa(14) + 1
         do ig = 1, ng14
            tauself = selffac(lay) * (selfref(indself(lay),ig) + selffrac(lay) * &
                 (selfref(indself(lay)+1,ig) - selfref(indself(lay),ig)))
            taufor =  forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         ind0(lay) = ((jp(lay)-13)*5+(jt(lay)-1))*nspb(14) + 1
         ind1(lay) = ((jp(lay)-12)*5+(jt1(lay)-1))*nspb(14) + 1
         do ig = 1, ng14
            taug(lay,ngs13+ig) = colco2(lay) * &
                 (fac00(lay) * absb(ind0(lay),ig) + &
                 fac10(lay) * absb(ind0(lay)+1,ig) + &
...
! Local 
      integer :: lay, ind0(nlayers), ind1(nlayers), ig
      integer, dimension(nlayers) :: js, js1, jmn2, jpl
      real(kind=r8), dimension(nlayers) :: speccomb, specparm, specmult, fs
      real(kind=r8), dimension(nlayers) :: speccomb1, specparm1, specmult1, fs1
      real(kind=r8), dimension(nlayers) :: speccomb_mn2, specparm_mn2, specmult_mn2, fmn2
      real(kind=r8), dimension(nlayers) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=r8) :: p, p4, fk0, fk1, fk2
      real(kind=r8), dimension(nlayers) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=r8), dimension(nlayers) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=r8) :: scalen2(nlayers), tauself, taufor, n2m1, n2m2, taun2 
      real(kind=r8) :: refrat_planck_a, refrat_m_a
      real(kind=r8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         speccomb(lay) = coln2o(lay) + rat_n2oco2(lay)*colco2(lay)
         specparm(lay) = coln2o(lay)/speccomb(lay)
         if (specparm(lay) .ge. oneminus) specparm(lay) = oneminus
         specmult(lay) = 8._r8*(specparm(lay))
         js(lay) = 1 + int(specmult(lay))
         fs(lay) = mod(specmult(lay),1.0_r8)

         speccomb1(lay) = coln2o(lay) + rat_n2oco2_1(lay)*colco2(lay)
         specparm1(lay) = coln2o(lay)/speccomb1(lay)
         if (specparm1(lay) .ge. oneminus) specparm1(lay) = oneminus
         specmult1(lay) = 8._r8*(specparm1(lay))
         js1(lay) = 1 + int(specmult1(lay))
         fs1(lay) = mod(specmult1(lay),1.0_r8)

         speccomb_mn2(lay) = coln2o(lay) + refrat_m_a*colco2(lay)
         specparm_mn2(lay) = coln2o(lay)/speccomb_mn2(lay)
         if (specparm_mn2(lay) .ge. oneminus) specparm_mn2(lay) = oneminus
         specmult_mn2(lay) = 8._r8*specparm_mn2(lay)
         jmn2(lay) = 1 + int(specmult_mn2(lay))
         fmn2(lay) = mod(specmult_mn2(lay),1.0_r8)

         speccomb_planck(lay) = coln2o(lay)+refrat_planck_a*colco2(lay)
         specparm_planck(lay) = coln2o(lay)/speccomb_planck(lay)
         if (specparm_planck(lay) .ge. oneminus) specparm_planck(lay)=oneminus
         specmult_planck(lay) = 8._r8*specparm_planck(lay)
         jpl(lay)= 1 + int(specmult_planck(lay))
         fpl(lay) = mod(specmult_planck(lay),1.0_r8)

         ind0(lay) = ((jp(lay)-1)*5+(jt(lay)-1))*nspa(15) + js(lay)
         ind1(lay) = (jp(lay)*5+(jt1(lay)-1))*nspa(15) + js1(lay)
      ...
      do lay = 1, laytrop
         do ig = 1, ng15
            ...
            taufor =  forfac(lay) * (forref(indfor(lay),ig) + forfrac(lay) * &
                 (forref(indfor(lay)+1,ig) - forref(indfor(lay),ig))) 
            n2m1 = ka_mn2(jmn2(lay),indminor(lay),ig) + fmn2(lay) * &
                 (ka_mn2(jmn2(lay)+1,indminor(lay),ig) - ka_mn2(jmn2(lay),indminor(lay),ig))
            n2m2 = ka_mn2(jmn2(lay),indminor(lay)+1,ig) + fmn2(lay) * &
                 (ka_mn2(jmn2(lay)+1,indminor(lay)+1,ig) - ka_mn2(jmn2(lay),indminor(lay)+1,ig))
            taun2 = scalen2(lay) * (n2m1 + minorfrac(lay) * (n2m2 - n2m1))
            ...
            taug(lay,ngs14+ig) = tau_major + tau_major1 &
                 + tauself + taufor &
                 + taun2
            fracs(lay,ngs14+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
      ...
      do lay = 1, laytrop
         ...
         do ig = 1, ng16
            ...
            taug(lay,ngs15+ig) = tau_major + tau_major1 &
                 + tauself + taufor
            fracs(lay,ngs15+ig) = fracrefa(ig,jpl(lay)) + fpl(lay) * &
                 (fracrefa(ig,jpl(lay)+1)-fracrefa(ig,jpl(lay)))
         enddo
      enddo
