...
!! part of the standard CAM distribution. Please contact Chuck Bardeen at
...
module carma_model_mod
  ...
  integer, public, parameter      :: NGROUP   = 5               !! Number of particle groups
  ...
  integer, public, parameter      :: I_DUST    = 4               !! dust
  ...
  integer, public, parameter      :: I_GRP_CRDUST   = 5             !! dust
  ...
  integer, public, parameter      :: I_ELEM_CRDCOR  = 5             !! dust core
  integer, public, parameter      :: I_ELEM_CRLIQ   = 6             !! water vapor
...
!  real(kind=f), parameter         :: dist_hym2010_beta    = -0.0538_f     !! beta (stratiform)
  ...
  logical, public, parameter      :: carma_do_initice    = .true.   ! If .true. then CARMA carma prognositic bins are set
                                                                    ! from the bulk ice on the first timestep
  logical, public, parameter      :: carma_do_bulk_tend  = .true.   ! If .true. then update CAM bulk tendencies
  logical, public, parameter      :: carma_do_autosnow    = .false. ! If .true. then the largest ice bin is autoconverted
                                                                    ! to snow at the end of the timestep.
...
contains
  ...
  subroutine CARMA_DefineModel(carma, rc)
    ...
    real(kind=f), parameter            :: rmin_dust  = 1.19e-5_f ! minimum radius (cm)
!    real(kind=f), parameter            :: vmrat_dust = 2.371_f   ! volume ratio
    real(kind=f), parameter            :: vmrat_dust = 1.588_f   ! volume ratio
        ...
        do j = 1, warren_nwave
          if (wave(i) <= warren_wave(j)) then
            if ((j > 1) .and. (wave(i) /= warren_wave(j))) then
              ...
              refidx_ice(i) = cmplx(warren_real(j), warren_imag(j),kind=f)
            endif
...
!    call rhoice_heymsfield2010(carma, RHO_I, I_GRP_CRDICE, "conv", rhoelem, arat, rc)
    ...
    call CARMAELEMENT_Create(carma, I_ELEM_CRDCOR, I_GRP_CRSICE, "Dust Core", RHO_DUST, &
         I_COREMASS, I_DUST, rc, shortname="CRDCOR")
    if (rc < RC_OK) call endrun('CARMA_DefineModel::CARMAElement_Create failed.')
...
!         I_AERFREEZE + I_AF_KOOP_2000 + I_AF_MURRAY_2010, 0._f, rc, igas=I_GAS_H2O, ievp2elem=I_ELEM_CRCN)
    ...
    call CARMA_AddNucleation(carma, I_ELEM_CRDUST, I_ELEM_CRDCOR, I_HETNUC, 0._f, rc, igas=I_GAS_H2O, ievp2elem=I_ELEM_CRDUST)
    if (rc < RC_OK) call endrun('CARMA_DefineModel::CARMA_AddNucleation failed.')
...
!    rliq(icol)     = 0._f
  ...
  2 format(/,'CARMA_DiagnoseBins::ERROR - CAM liquid mass conservation error, icol=',i4,', iz=',i4,',lat=',&
    ...
    if ((.not. carma_do_cldice) .or. (is_first_step() .and. carma_do_initice)) then
      ...
      call CARMA_GetGammaParmsForIce(carma, state%q(icol, :, ixcldice), &
           state%q(icol, :, ixnumice), rhoa_wet(:), mu(:), lambda(:), rc)
      if (rc < RC_OK) call endrun('CARMA_DiagnoseBins::CARMA_GetGammaParmsForIce failed.')
  
      call CARMA_GetMmrFromGamma(carma, r(:), dr(:), rmass(:), &
           state%q(icol, :, ixcldice), state%q(icol, :, ixnumice), mu(:), &
           lambda(:), mmr(:, :), rc)
  ...
  subroutine CARMA_InitializeModel(carma, lq_carma, rc)
    ...
    !
    ! TBD: This should us the radii rather than being hard coded.
    ! nClay = 8
    ! nSilt = NBIN - nClay   
    do ielem = 1, NELEM    
...
!    real(r8), parameter                :: qsmall     = 1.e-18_r8     ! min mixing ratio 
  ...
  !!  @version Nov-2009 
  !!  @author  Chuck Bardeen 
  subroutine CARMA_GetTotalIceAndSnow(carma, cstate, makeSnow, iceMass, iceNumber, &
       snowMass, snowNumber, snowSurface, rc, iceRe)
    implicit none
  ...
  1 format(/,'CARMA_CheckMassAndEnergy::ERROR - CARMA mass conservation error, ',a,',icol=',i4,',lat=',&
              f7.2,',lon=',f7.2,',cam=',e17.10,',carma=',e17.10,',diff=',e17.10,',rer=',e10.3)
    ...
    if (abs((totalMass - state%tw_cur(icol))) / state%tw_cur(icol) > 1e14_f) then
      if (do_print) then
         write(LUNOPRT,*) "CARMA_CheckMassAndEnergy::&
              &WARNING Total water not conserved, ", totalMass, state%tw_cur, &
              (totalMass - state%tw_cur(icol)), &
  ...
  subroutine CARMA_ClayMassFraction(carma, igroup, rc)
    ...
    integer             :: count(NBIN_TEGEN+2)             ! count number
    ...
    ind_up(:)   = 0
    ind_low(:)  = 0
    ind_up (IBELOW)  = count(IBELOW)
    ind_low(IBELOW)  = min(1, count(IBELOW))
    
    do j = 1, 5
      ind_up (j+1) = ind_up(j) + count(j+1)
      ind_low(j+1) = ind_up(j) + min(count(j+1), 1)
    end do
    ...
    do j = 1, NBIN_TEGEN
      if (count(j+1) > 0) then
        clay_mf(ind_low(j+1):ind_up(j+1)) = tl_mf(j) / count(j+1)
      end if
    end do 
  ...
  !! model.
  !!
  !! NOTE: This is currently doing 2-D interpolation, but it really wshould be doing
  !! regridding.
  !!
  !! @author  Pengfei Yu
  !! @version July-2012
