...
!
#ifdef CLUBB_CRM
                    wwqui_cen_sum(k) = wwqui_cen_sum(k)+mask * ((ww(i,j,k)+ww(i,j,k+1))*0.5-wwqui_bar_cen(k))**2 + mask * &
                                       (wwsq(i,j,k)+wwsq(i,j,k+1))**2/4. 
#else
                    wwqui_cen_sum(k) = wwqui_cen_sum(k)+mask * ((ww(i,j,k)+ww(i,j,k+1))*0.5-wwqui_bar_cen(k))**2 + mask * &
                                       tkesgs(i,j,k)/3.
#endif
                    if(icl.eq.CLD) then
#ifdef CLUBB_CRM
                      wwqui_cloudy_cen_sum(k)=wwqui_cloudy_cen_sum(k)+mask * ((ww(i,j,k)+ww(i,j,k+1))*0.5-wwqui_bar_cen(k))**2 &
                                              + mask * (wwsq(i,j,k)+wwsq(i,j,k+1))**2/4.
#else
                      wwqui_cloudy_cen_sum(k)=wwqui_cloudy_cen_sum(k)+mask * ((ww(i,j,k)+ww(i,j,k+1))*0.5-wwqui_bar_cen(k))**2 &
                                              + mask * tkesgs(i,j,k)/3.
#endif
...
#else
                    wwqui_bnd_sum(k) = wwqui_bnd_sum(k)+mask * (ww(i,j,k)-wwqui_bar_bnd(k))**2 + mask * (tkesgs(i,j,km0)+&
                                       tkesgs(i,j,km1)) * 0.5/3.
#endif
                    if(icl.eq.CLD) then
#ifdef CLUBB_CRM
                      wwqui_cloudy_bnd_sum(k)=wwqui_cloudy_bnd_sum(k)+mask * (ww(i,j,k)-wwqui_bar_bnd(k))**2 + mask * &
                                              wwsq(i,j,k)**2 
#else
                      wwqui_cloudy_bnd_sum(k)=wwqui_cloudy_bnd_sum(k)+mask * (ww(i,j,k)-wwqui_bar_bnd(k))**2 + mask * &
                                              (tkesgs(i,j,km0)+tkesgs(i,j,km1)) * 0.5/3.
#endif
                    end if
...
! testing small queiscent fraction +++mhwang
  do k=1, nz
    if(sum(area_cen_final(k,:,1,:)).lt.1.0e-3) then
      write(0, *) 'ecpp, area_cen_final, quiescent', sum(area_cen_final(k,:,1,:)), k, area_cen_final(k,:,1,:), &
                   wdown_thresh_k(k,1), wup_thresh_k(k,1)
...
#endif /*ECPP*/
end module crmx_module_ecpp_stats
