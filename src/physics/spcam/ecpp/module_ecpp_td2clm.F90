...
!   save values in these arrays
	...
	chem_sub_new(:,:,:,:) = chem_sub_beg(:,:,:,:)
        ...
        del_activate3d(:,:,:,:) = 0.0_r8

!   calc "area*rho*dz" and "area*rho" arrays
	ardz_cen_tavg(:,:,:) = 0.0_r8
	ardz_cen_tfin(:,:,:) = 0.0_r8
	ar_bnd_tavg(:,:,:) = 0.0_r8
	do k = kts, ktebnd
	    do icc = 0, 2
...
!
        if (ktau <= 1) then
            tmpvecsva(:) = 0.0_r8 ; tmpvecsvb(:) = 0.0_r8 ; tmpvecsvc(:) = 0.0_r8
        end if
        tmpveca(:) = 0.0_r8
        do jcls = 1, ncls_use
        do icc = 1, 2
...
!   (area*rho*dz) is fixed at ardz_cen_new during the integration loop
		...
		if (tmp_ardz < ardz_cut) then
		    tmpcourout = 0.0_r8
		else if (tmpb > 1.0e3_r8*tmp_ardz) then
		    tmpcourout = 1.0e3_r8
		else
...
!    Minghuai Wang, 2010-05
            ...
            if(mtype_updnenv_use(icc, jcls) == mtype_updraft_ecpp) then
              abnd_up = 0.0_r8
              abnd_dn = 0.0_r8
              if(rhobnd_bar(k+1).gt.1.0e-10_r8) then
                abnd_up = ar_bnd_tavg(k+1, icc, jcls)/rhobnd_bar(k+1)
              end if
              if(rhobnd_bar(k).gt.1.0e-10_r8) then
                abnd_dn = ar_bnd_tavg(k, icc, jcls)/rhobnd_bar(k)
              end if
              if(k.eq.kts) then
                frc_ent_act = 1.0_r8
              else if(abnd_up.gt.1.0e-5_r8) then
                frc_ent_act = 1.0_r8 - min(1.0_r8, abnd_dn/abnd_up)

                if(mfbnd_use(k+1, icc, jcls).gt.1.0e-20_r8) then
...
entrain_iccy_loop:   &
		...
		if (tmp_del_ardz <= 0.0_r8) cycle entrain_iccy_loop
...
!			tmp_fmnact = fmact_horz(m,n,jcls,iccy,jclsy,k)
			...
			tmp_del_ardzqa = (tmp_qyla+tmp_qylc)*(1.0_r8-tmp_fmnact)*tmp_del_ardz
...
vert_botqu_iccy_loop:   &
		do iccy = 1, 2
		    ...
		    if (tmp_del_ardz == 0.0_r8) cycle vert_botqu_iccy_loop
...
vert_topqu_iccy_loop:   &
		do iccy = 1, 2
		    ...
		    if (tmp_del_ardz == 0.0_r8) cycle vert_topqu_iccy_loop
...
!		    tmpc = tmpc + tmp_qylc*tmpd
		    ...
		    if (ido_actres_tmp == 2) then
!			tmpa = tmpa + (tmp_qyla+tmp_qylc)*(1.0_r8-tmp_fmnact)*tmpd
!			tmpc = tmpc + (tmp_qyla+tmp_qylc)*(tmp_fmnact    )*tmpd
                       tmp_del_ardzqa = (tmp_qyla+tmp_qylc)*(1.0_r8-tmp_fmnact)*tmpd
...
!   ***************************************************************
	...
	do icc = 1, 2
		...
		dcls_aalimit(icc,jcls) = 1.0e30_r8
	    end if
	end do
	end do
...
!	when iflag_ecpp_startup_host_chemtend > 0
	if (iflag_ecpp_startup_host_chemtend > 0) then
		...
		if (tmpqbarold < 1.01_r8*max(epsilc,1.0e-20_r8)) then
...
acwxx1_k_loop: &
	do k = kts, ktecen
		...
		do la = p1st, num_chem_ecpp
		    ...
		    chem_sub_beg(k,1:2,jcls,lc) = 0.0_r8
		    chem_cls(k,jcls,la) = tmpa
		    chem_cls(k,jcls,lc) = 0.0_r8
		end do ! la
		cycle acwxx1_k_loop
	    end if
...
!		if ((k <= 5) .and. (isize == 1) .and. (ll == 3)) then
	    ...
	    qv_a_x = qv_a_x*1.0e-6_r8   ! because mass mixratios are ug/kg,
	    qv_a_y = qv_a_y*1.0e-6_r8   ! and want volume mixratio in cm3-aerosol/kg
...
! if this has occurred, then partition the number differently
		...
		if ((itmpa==5) .and. (qv_a_y>0.0_r8)) itmpa=8
		if ((itmpa==5) .and. (qn_a_y>0.0_r8)) itmpa=9
		if (lun164 > 0) then
...
!       set areas = 0 for drafts (at kts set abnd=0 but allow acen>0)
	do k = kts, ktebnd
		...
		if (jcls == jcls_qu) then
		    ...
		    abnd_tavg_use(k,0:2,jcls) = 0.0_r8
		    if ((k == kts) .or. (k > ktecen)) cycle
		    acen_tavg_use(k,0:2,jcls) = 0.0_r8
...
!       set acen_tavg to zero if abnd_tavg=0 at both layer boundaries (14-apr-2009)
	do k = kts, ktecen
		...
		tmpe = 0.5_r8*( abnd_tavg_use(k,  icc,jcls) + &
...
!   *** NOTE / TODO - in the "new" code, this may not work correctly
	...
	do k = kts, ktebnd
		...
		do jcls = 1, ncls_use
		    ...
		    abnd_tavg_use(k,icc,jcls   ) = 0.0_r8

		    mfbnd_use(    k,icc,jcls_qu) = mfbnd_use(    k,icc,jcls_qu)   &
		                                 + mfbnd_use(    k,icc,jcls   )
		    mfbnd_use(    k,icc,jcls   ) = 0.0_r8

		    if (k > ktecen) cycle

		    acen_tavg_use(k,icc,jcls_qu) = acen_tavg_use(k,icc,jcls_qu)   &
		                                 + acen_tavg_use(k,icc,jcls   )
		    acen_tavg_use(k,icc,jcls   ) = 0.0_r8

		    acen_tfin_use(k,icc,jcls_qu) = acen_tfin_use(k,icc,jcls_qu)   &
		                                 + acen_tfin_use(k,icc,jcls   )
		    acen_tfin_use(k,icc,jcls   ) = 0.0_r8
		end do
	    end do
	end do
...
!   *** currently the reset of mtype_updnenv_use is deactivated
	...
	do icc = 1, 2
	    ...
	    itmpb = 0
	    do k = kts, ktebnd
		if (mfbnd_use(k,icc,jcls)    /= 0.0_r8) itmpa = itmpa + 1
		if (abnd_tavg_use(k,icc,jcls) /= 0.0_r8) itmpb = itmpb + 1
	    end do
...
!	write(lun,9400) 'output from ppboxmakeinp01'
	...
	do k = ktebnd, kts, -1
	    if (k < ktebnd) then
	      duma = zbnd(k) + 0.5_r8*dzcen(k)
	      write(lun,'(i2,2x,f8.3,f8.1,f8.4,f8.1,  8x)')   &
		k, duma*1.0e-3_r8, pcen_bar(k)*1.0e-2_r8, rhocen_bar(k), tcen_bar(k)-273.16_r8
	    end if
	    duma = k-0.5_r8
	    write(lun,'( f4.1,f8.3,  8x,f8.4,  8x,f8.2)')   &
		duma, zbnd(k)*1.0e-3_r8, rhobnd_bar(k), wbnd_bar(k)*1.0e2_r8
	end do
	write(lun,'(a)') ' k     z(km)   p(mb)     rho    t(C) w(cm/s)'
	...
	do iclrcld = 1, 2
		...
		if (duma == 0.0_r8) then
		    dumchar8(k) = '  0.    '
		else if (abs(duma) >= 5.0e-5_r8) then
...
!	    write(lun,'(1p,15e8.1 )') (abnd_tavg(k,iclrcld,jcls), k=kts,ktebnd)
	...
	write(lun,'(1p,12e10.2)') ((dumarr2(k)-1.0_r8), k=kts,ktebnd)
	write(lun,'(a)') '(acen_tavg-1) summed over all subs'
	write(lun,'(1p,12e10.2)') ((dumarr3(k)-1.0_r8), k=kts,ktecen)
	write(lun,'(a)') '(acen_tbeg-1) summed over all subs'
	write(lun,'(1p,12e10.2)') ((dumarr4(k)-1.0_r8), k=kts,ktecen)
	write(lun,'(a)') '(acen_tfin-1) summed over all subs'
	write(lun,'(1p,12e10.2)') ((dumarr5(k)-1.0_r8), k=kts,ktecen)
