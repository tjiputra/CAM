...
!-----------------------------------------------------------------------
	...
	use module_data_ecpp1
        ...
        use chem_mods,      only: gas_pcnst, nfs, indexm
...
!   local variables
	...
	integer :: p1st
        ...
        integer :: ncol
        integer :: empty_troplev(pcols) = -99  ! This variable is not used in the modal_aero_rename_no_acc_crs_sub (which is
                                               ! called witin modal_aero_rename_sub) when moal_accum_coars_exch is false

	real(r8) :: tmpa, tmpa1, tmpa2, tmpb1, tmpb2, tmpq, tmpq1, tmpq2, tmpx, tmpx2, tmpy, tmpy2
        real(r8) :: dtmpchem

	real(r8), parameter :: qcldwtr_cutoff = 1.0e-6_r8
        real(r8), parameter :: yph = 4.5_r8 ! in the MMF model, for ECPP, ph value is fixed at 4.5
	real(r8) :: dt_tmp
	...
	real(r8), allocatable :: chem_tmpa(:,:,:,:), chem_tmpb(:,:,:,:), chem_tmpc(:,:,:,:)
        ...
        real(r8), allocatable :: aqso4_tmp(:,:)
        real(r8), allocatable :: aqh2so4_tmp(:,:)
...
!  so convert it to from top to the bottom for aqueous chemistry at CAM.
	  ...
	  alt_tmp(1:4,k,1:ncls_use) = 1.0_r8/rhocen_bar(km)
	...
	if (cldchem_onoff_ecpp > 0) then
	  ...
	  vmrcw_3d(1, :, :) = vmrcw(:, :)
          ...
          call setsox( ncol,   &                   ! ncol
               ...
               imozart-1,&                         ! loffset
               dt_tmp,                  &          ! dtime
               p_tmp(iccpp:iccpp, :, jcls),   &    ! press
               pdel_tmp(iccpp:iccpp, :, jcls),   & ! pdel
               t_tmp(iccpp:iccpp, :, jcls),   &    ! tfld
               mbar_3d,   &                        ! mbar(1,:)
			...
			tmpy = tmpy + tmpa*tmpq
                    ...
                    if(rename_onoff_ecpp > 0 ) then
                      if (tmpa > afrac_cut_0p5) then
                        ...
                        del_rename3d(k,icc,jcls,ipp,l)=del_rename3d(k,icc,jcls,ipp,l)+0.0_r8
                      end if
                    end if  ! (rename_onoff_ecpp > 0.)
