...
! 
! Entry points:
!      phys_grid_readnl    read namelist options
!
...
subroutine phys_grid_readnl(nlfile)
   ...
   namelist /phys_grid_nl/ phys_alltoall, phys_loadbalance, phys_twin_algorithm, &
                           phys_chnk_per_thd
   !-----------------------------------------------------------------------------

   ! Initialize namelist vars
   phys_loadbalance = def_lbal_opt

   if (dycore_is('UNSTRUCTURED')) then
      phys_twin_algorithm = def_twin_alg_unstructured
   else
      phys_twin_algorithm = def_twin_alg_lonlat
   endif

   phys_chnk_per_thd = def_chunks_per_thread
   ...
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: phys_alltoall")
   call mpi_bcast(phys_loadbalance, 1, mpi_integer, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: phys_loadbalance")
   call mpi_bcast(phys_twin_algorithm, 1, mpi_integer, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: phys_twin_algorithm")
   call mpi_bcast(phys_chnk_per_thd, 1, mpi_integer, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: phys_chnk_per_thd")

   ! set module variables from namelist vars
   ...
   twin_alg = phys_twin_algorithm

   chunks_per_thread = phys_chnk_per_thd
...
# endif
      ...
      if (masterproc) then
         write(iulog,*) sub//': ERROR: phys_alltoall=', phys_alltoall, &
            '  is out of range.  It must be between ', min_alltoall, ' and ', max_alltoall
      endif
      call endrun(sub//': ERROR setting phys_alltoall')
   endif
#if defined(SPMD)
#if defined(MODCM_DP_TRANSPOSE)
   ...
   if ((lbal_opt < min_lbal_opt).or.(lbal_opt > max_lbal_opt)) then
      if (masterproc) then
         write(iulog,*) sub//': ERROR: phys_loadbalance=', phys_loadbalance, &
            '  is out of range.  It must be between ', min_lbal_opt, ' and ', max_lbal_opt
      endif
      call endrun(sub//': ERROR setting phys_loadbalance')
   endif

   if ((twin_alg < min_twin_alg).or.(twin_alg > max_twin_alg)) then
      if (masterproc) then
         write(iulog,*) sub//': ERROR: phys_twin_algorithm=', phys_twin_algorithm, &
            '  is out of range.  It must be between ', min_twin_alg, ' and ', max_twin_alg
      endif
      call endrun(sub//': ERROR setting phys_twin_algorithm')
   endif

   if (chunks_per_thread < min_chunks_per_thread) then
      if (masterproc) then
         write(iulog,*) sub//': ERROR: phys_chnk_per_thd=', phys_chnk_per_thd, &
            ' is too small.  It must not be smaller than ', min_chunks_per_thread
      endif
      call endrun(sub//': ERROR setting phys_chnk_per_thd')
   endif
