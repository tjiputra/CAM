...
subroutine radiation_readnl(nlfile)
   ...
   if (mod(iradae,iradlw)/=0) then
      write(iulog,*) subname//': iradae must be an even multiple of iradlw.'
      write(iulog,*)'     iradae = ',iradae,', iradlw = ',iradlw
      call endrun(subname//': iradae must be an even multiple of iradlw.')
   end if
...
!===============================================================================
  ...
  subroutine radiation_tend(state, ptend, pbuf, &
    ...
    type(rad_diagdata_type), intent(out) :: rd  ! structure to hold diagnostic data for output
    ...
    ! Optional variables
    logical, intent(in), optional :: do_output ! turn off output
    ...
    logical :: write_output ! turn off output
