...
!AL
     ...
     nprc1o, ncsedteno, nisedteno, nmelto, nhomoo,    &
...
!AL
 real(r8), intent(out) :: nnuccco(:,:)   ! immersion freezing
 real(r8), intent(out) :: nnuccto(:,:)   ! contact freezing
 real(r8), intent(out) :: npsacwso(:,:)  ! accr. snow
 real(r8), intent(out) :: nsubco(:,:)    ! evaporation of droplet 
 real(r8), intent(out) :: nprao(:,:)     ! accretion
 real(r8), intent(out) :: nprc1o(:,:)    ! autoconversion
 real(r8), intent(out) :: ncsedteno(:,:) ! nqc sedimentation tendency
 real(r8), intent(out) :: nisedteno(:,:) ! nqi sedimentation tendency
 real(r8), intent(out) :: nmelto(:,:)    ! nc melting of cloud ice
 real(r8), intent(out) :: nhomoo(:,:)    ! nc homogeneos freezign cloud water
 real(r8), intent(out) :: nimelto(:,:)   ! ni melting of cloud ice
 real(r8), intent(out) :: nihomoo(:,:)   ! ni homogeneos freezign cloud water
 real(r8), intent(out) :: nsacwio(:,:)   ! numb conc tend due to HM ice multiplication
 real(r8), intent(out) :: nsubio(:,:)    ! evaporation of cloud ice number (sublimation?)
 real(r8), intent(out) :: nprcio(:,:)    ! numb conc tend due to auto of cloud ice to snow
 real(r8), intent(out) :: npraio(:,:)    ! numb conc tend due to accr of cloud ice by snow
 real(r8), intent(out) :: nnudepo(:,:)   ! deposition?
 real(r8), intent(out) :: npccno(:,:)    ! droplet activation
 real(r8), intent(out) :: nnuccdo(:,:)   ! ni nucleation
 real(r8), intent(out) :: mnudepo(:,:)   ! deposition (mass)
!
 real(r8), intent(out) :: nctnszmx(:,:)  ! nc tuning: maximum slope (reduction of number)
 real(r8), intent(out) :: nctnszmn(:,:)  ! nc tuning: minimum slope (increase of numer)
 real(r8), intent(out) :: nctnncld(:,:)  ! nc tuning: removal of nc when qc is zero after mg
 real(r8), intent(out) :: nitncons(:,:)  ! ni tuning to conserve numberi in substeps
 real(r8), intent(out) :: nitnszmx(:,:)  ! ni tuning: maximum slope
 real(r8), intent(out) :: nitnszmn(:,:)  ! ni tuning: minimum minimum slope
 real(r8), intent(out) :: nitnncld(:,:)  ! ni tuning: removal of ni when qi is zero after mg
!AL
